...
def main(args, config):
    ...
    # model
    model = Gen(config=config, tokenizerSP=tokenizer,tokenizerSD=tokenizerSD, loader_len=len(data_loader) // torch.cuda.device_count())
    if args.checkpoint:
        checkpoint = torch.load(args.checkpoint, map_location='cpu')
        _ = model.load_state_dict(checkpoint['state_dict'], strict=False)
    # for param in model.parameters():   #param frozen
