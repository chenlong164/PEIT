...
def generate(model, text_embeds, Des, stochastic=False, text_att_mask=None, k=None):
    Des_atts = torch.where(Des == 0, 0, 1)
    if text_att_mask is None:
        text_att_mask = torch.ones(text_embeds.size(0), text_embeds.size(1), dtype=torch.long).to(text_embeds.device)
    # print("Des_embeds  ",Des_embeds.shape,"   text  ",text.shape,"   text_atts  ",text_atts.shape,"  Des_att_mask  ",Des_att_mask.shape)
    token_output = model.Des_encoder(Des,
                                      attention_mask=Des_atts,
                                      encoder_hidden_states=text_embeds,
                                      encoder_attention_mask=text_att_mask,
                                      return_dict=True,
                                      is_decoder=True,
                                      return_logits=True,
                                      )[:, -1, :]  # batch*300
...
@torch.no_grad()
def generate_with_Description(model, Description, n_sample, k=2, stochastic=True):
    device = model.device
    tokenizer = model.SDtokenizer
    # test
    model.eval()
    ...
    Description = tokenizer(Description, padding='longest', truncation=True, max_length=100, return_tensors="pt")
    Des_embeds = model.Des_encoder(inputs_embeds=Description, return_dict=True).last_hidden_state  # batch*len(=patch**2+1)*feature

    candidate = []
    for _ in tqdm(range(n_sample)):
        product_input = torch.tensor([tokenizer.cls_token_id]).expand(1, 1).to(device)
        values, indices = generate(model, Des_embeds, product_input, stochastic=stochastic, k=k)
...
def metric_eval(prop_input, cand, mask):
    ...
    with open('modelpth/generated_molecules.txt', 'w') as w:
...
def main(args, config):
    ...
    print("=" * 50)
    samples = generate_with_Description(model, Des_input, args.n_generate, stochastic=args.stochastic, k=args.k)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='./Pretrain')
