...
from transformers import BertTokenizer, WordpieceTokenizer, BertModel
import datetime
...
class SANDT_classifier(nn.Module):
    def __init__(self, tokenizer=None, config=None):
        super().__init__()
        self.tokenizer = tokenizer
        bert_config = BertConfig.from_json_file(config['bert_config_text'])
        self.text_encoder = BertModel.from_pretrained("sci_bert")
        for i in range(bert_config.fusion_layer, bert_config.num_hidden_layers):  self.text_encoder.encoder.layer[i] = nn.Identity()
    ...
    def forward(self, text_input_ids, text_attention_mask, value, eval=False):
        vl_embeddings = self.text_encoder(text_input_ids, text_attention_mask, return_dict=True).last_hidden_state[:, 0, :]
...
def main(args, config):
    ...
    tokenizer = BertTokenizer.from_pretrained("sci_bert")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='./modelpth/checkpoint_SDPFusion_epoch=6.ckpt')
